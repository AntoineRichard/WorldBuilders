...
def PlaneMeshConverter(prim: Usd.prim) -> Plane_T:
    ...
    ymax = 0.5 * scale[1]
    T = getTransform(prim)
    return Plane_T(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, transform=T, output_space=3)
...
def CubeMeshConverter(prim: Usd.Prim) -> Cube_T:
    ...
    zmax = 0.5 * scale[2]
    T = getTransform(prim)
    return Cube_T(xmin=xmin, xmax=xmax, ymin=ymin, ymax=ymax, zmin=zmin, zmax=zmax, transform=T, output_space=3)
