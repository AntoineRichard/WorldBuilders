...
class BaseLayer:
    ...
    def __init__(self, layer_cfg: Layer_T, sampler_cfg: Sampler_T, **kwarg) -> None:
        ...
        assert self.__class__.__name__[:-5] == layer_cfg.__class__.__name__[:-2], (
            "Configuration mismatch. The type of the config must match the type of the layer."
        )
    ...
    def buildRotMat3DfromQuat(xyzw: list) -> np.ndarray([3, 3], dtype=float):
        ...
        return 2 * np.array(
            [
                [q0 * q0 + q1 * q1, q1 * q2 - q0 * q3, q1 * q3 + q0 * q2],
                [q1 * q2 + q0 * q3, q0 * q0 + q2 * q2, q2 * q3 - q0 * q1],
                [q1 * q3 - q0 * q2, q2 * q3 + q0 * q1, q0 * q0 + q3 * q3],
            ]
        ) - np.eye(3)
    ...
    def buildRotMat3DfromEuler(xyz: list) -> np.ndarray([3, 3], dtype=float):
        ...
        return np.array(
            [
                [cy * cz, sx * sy * cz - cx * sz, cx * sy * cz + sx * sy],
...
class ImageLayer(Layer2D):
    ...
    def getBounds(self) -> None:
        ...
        self._bounds = np.array([[0, self.xmax], [0, self.ymax]])
...
class LineLayer(Layer1D):
    ...
    def __init__(self, layer_cfg: Line_T, sampler_cfg: Sampler_T) -> None:
        ...
        if isinstance(self._sampler_cfg, UniformSampler_T) or isinstance(
            self._sampler_cfg, LinearInterpolationSampler_T
        ):
...
class CircleLayer(Layer1D):
    ...
    def project(self, points: np.ndarray([])) -> np.ndarray([]):
        ...
        if self._skip_projection:
            ...
            zeros = np.zeros(
                [points.shape[0], self._layer_cfg.output_space - self._sampler_cfg.randomization_space - 1]
            )
            points = np.concatenate([points, zeros], axis=-1)
            return points
...
class PlaneLayer(Layer2D):
    ...
    def getBounds(self) -> None:
        ...
        self._bounds = np.array(
            [[self._layer_cfg.xmin, self._layer_cfg.xmax], [self._layer_cfg.ymin, self._layer_cfg.ymax]]
        )
...
class DiskLayer(Layer2D):
    ...
    def __init__(self, layer_cfg: Disk_T, sampler_cfg: Sampler_T) -> None:
        ...
        if isinstance(self._sampler_cfg, UniformSampler_T) or isinstance(
            ...
            tmp1 = (self._sampler_cfg.mean[0] - self._layer_cfg.radius_min) / (
                self._layer_cfg.radius_max - self._layer_cfg.radius_min
            )
            tmp2 = (self._sampler_cfg.mean[1] - self._layer_cfg.theta_min) / (
                self._layer_cfg.theta_max - self._layer_cfg.theta_min
            )
    ...
    def getBounds(self) -> None:
        ...
        self._area = (self._layer_cfg.theta_max - self._layer_cfg.theta_min) * (
            self._layer_cfg.radius_max - self._layer_cfg.radius_min
        ) ** 2
    ...
    def sample(self, num: int = 1, **kwargs) -> np.ndarray([]):
        ...
        r_rescaled = (
            self._layer_cfg.radius_min + rand[:, 0] * (self._layer_cfg.radius_max - self._layer_cfg.radius_min)
        ) / self._layer_cfg.radius_max
...
class CubeLayer(Layer3D):
    ...
    def getBounds(self) -> None:
        ...
        self._bounds = np.array(
            [
                [self._layer_cfg.xmin, self._layer_cfg.xmax],
...
class RollPitchYawLayer(Layer4D):
    ...
    def getBounds(self) -> None:
        ...
        self._bounds = np.array(
            [
                [self._layer_cfg.rmin, self._layer_cfg.rmax],
...
class SphereLayer(Layer3D):
    ...
    def __init__(self, layer_cfg: Sphere_T, sampler_cfg: Sampler_T) -> None:
        ...
        if isinstance(self._sampler_cfg, UniformSampler_T) or isinstance(
            ...
            tmp2 = (self._sampler_cfg.mean[1] - self._layer_cfg.phi_min) / (
                self._layer_cfg.phi_max - self._layer_cfg.phi_min
            )
            tmp3 = (self._sampler_cfg.mean[2] - self._layer_cfg.theta_min) / (
                self._layer_cfg.theta_max - self._layer_cfg.theta_min
            )
    ...
    def getBounds(self) -> None:
        ...
        self._bounds = np.array([[0.0, 1.0], [0.0, 1.0], [0.0, 1.0]])
        self._area = (
            (self._layer_cfg.theta_max - self._layer_cfg.theta_min)
            * (self._layer_cfg.phi_max - self._layer_cfg.phi_min)
            * (self._layer_cfg.radius_max - self._layer_cfg.radius_min) ** 2
        )
...
class CylinderLayer(Layer3D):
    ...
    def __init__(self, layer_cfg: Cylinder_T, sampler_cfg: Sampler_T) -> None:
        ...
        if isinstance(self._sampler_cfg, UniformSampler_T) or isinstance(
            ...
            tmp2 = (self._sampler_cfg.mean[1] - self._layer_cfg.height_min) / (
                self._layer_cfg.height_max - self._layer_cfg.height_min
            )
            tmp3 = (self._sampler_cfg.mean[2] - self._layer_cfg.theta_min) / (
                self._layer_cfg.theta_max - self._layer_cfg.theta_min
            )
...
class ConeLayer(Layer3D):
    ...
    def sample(self, num: int = 1, **kwargs) -> np.ndarray([]):
        ...
        z = (
            self._layer_cfg.center[2]
            + self._layer_cfg.height_min
            + h * (self._layer_cfg.height_max - self._layer_cfg.height_min)
        )
        return np.stack([x, y, z]).T
...
class TorusLayer(Layer3D):
    ...
    def __init__(self, layer_cfg: Torus_T, sampler_cfg: Sampler_T) -> None:
        ...
        if isinstance(self._sampler_cfg, UniformSampler_T) or isinstance(
            ...
            tmp1 = (self._sampler_cfg.mean[0] - self._layer_cfg.radius2_min) / (
                self._layer_cfg.radius2_max - self._layer_cfg.radius2_min
            )
            tmp2 = (self._sampler_cfg.mean[1] - self._layer_cfg.theta1_min) / (
                self._layer_cfg.theta1_max - self._layer_cfg.theta1_min
            )
            tmp3 = (self._sampler_cfg.mean[2] - self._layer_cfg.theta2_min) / (
                self._layer_cfg.theta2_max - self._layer_cfg.theta2_min
            )
            self._sampler_cfg.mean = np.array([tmp1, tmp2, tmp3])
    ...
    def getBounds(self) -> None:
        ...
        self._area = (
            (self._layer_cfg.theta1_max - self._layer_cfg.theta1_min)
            * (self._layer_cfg.theta2_max - self._layer_cfg.theta2_min)
            * (self._layer_cfg.radius1)
            * (self._layer_cfg.radius2_max - self._layer_cfg.radius2_min) ** 2
        )
    ...
    def sample(self, num: int = 1, **kwargs) -> np.ndarray([]):
        ...
        r2_rescaled = (
            self._layer_cfg.radius2_min + rand[:, 0] * (self._layer_cfg.radius2_max - self._layer_cfg.radius2_min)
        ) / self._layer_cfg.radius2_max
        ...
        x = (
            self._layer_cfg.center[0]
            + self._layer_cfg.radius1 * np.cos(t)
            + r2 * np.cos(t) * np.sin(p) * self._layer_cfg.alpha
        )
        y = (
            self._layer_cfg.center[1]
            + self._layer_cfg.radius1 * np.sin(t)
            + r2 * np.sin(t) * np.sin(p) * self._layer_cfg.beta
        )
