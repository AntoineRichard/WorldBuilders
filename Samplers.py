...
class UniformSampler(BaseSampler):
    ...
    def sample(self, num=1, **kwargs):
        points = np.stack(
            [
                self._rng.uniform(self._sampler_cfg.min[dim], self._sampler_cfg.max[dim], (num))
                for dim in range(self._sampler_cfg.randomization_space)
            ]
        ).T
...
class MaternClusterPointSampler(BaseSampler):
    ...
    def getParents(self, bounds, area=None):
        ...
        for i in range(bounds_ext.shape[0]):
            coords.append(
                bounds_ext[i, 0] + (bounds_ext[i, 1] - bounds_ext[i, 0]) * self._rng.uniform(0, 1, num_points_parent)
            )
...
class LinearInterpolationSampler(BaseSampler):
    ...
    def sample(self, num: int = 1, endpoint: tuple = None, **kwargs):
        ...
        grids = np.meshgrid(
            *[
                np.linspace(self._sampler_cfg.min[dim], self._sampler_cfg.max[dim], num_per_dim, endpoint=endpoint[dim])
                for dim in range(self._sampler_cfg.randomization_space)
            ]
        )
    ...
    def sample_equation_based_rejection(self, num=1, **kwargs):
        ...
        for i in range(num):
            ...
            while check_not_ok:
                point = [
                    self._rng.uniform(self._sampler_cfg.min[dim], self._sampler_cfg.max[dim])
                    for dim in range(self._sampler_cfg.randomization_space)
                ]
...
class DeterministicSampler(BaseSampler):
    ...
    def load_data(self):
        ...
        assert self.data.shape[-1] == self._sampler_cfg.randomization_space, (
            f"point's dimension does not match with {self._sampler_cfg.randomization_space}"
        )
