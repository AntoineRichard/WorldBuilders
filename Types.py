...
class Polygon_T(Layer_T):
    ...
    def __post_init__(self):
        ...
        assert self.use_rejection_sampling or self.use_image_sampling, (
            "Must used either rejection sampling or image sampling."
        )
...
class Sphere_T(Layer_T):
    ...
    def __post_init__(self):
        super().__post_init__()
        if self.transform is not None:
            assert isinstance(self.transform, Transformation3D_T), (
                "A 3D object, like a Shpere, requires a 3D transform."
            )
        assert self.output_space >= 3, "output_space must be greater or equal to 3."
        assert self.alpha > 0, "The alpha value must be larger than 0."
        assert self.beta > 0, "The beta value must be larger than 0."
...
class Cylinder_T(Layer_T):
    ...
    def __post_init__(self):
        ...
        if self.transform is not None:
            assert isinstance(self.transform, Transformation3D_T), (
                "A 3D object, like a Cylinder, requires a 3D transform."
            )
        assert self.output_space >= 3, "output_space must be greater or equal to 3."
        assert self.alpha > 0, "The alpha value must be larger than 0."
        assert self.beta > 0, "The beta value must be larger than 0."
...
class MaternClusterPointSampler_T(InterativePointProcess_T):
    ...
    def __post_init__(self):
        ...
        if self.warp is not None:
            assert len(self.warp) == self.randomization_space, (
                "warp parameter must be of same length as the randomization space."
            )
