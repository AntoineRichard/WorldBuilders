...
def assembleMap(folder, stage, terrain_root="/terrain", texture_path="../../../Textures/Sand.mdl", texture_name="Sand"):
    ...
    loadTexture(stage, texture_path, texture_name, terrain_root + "/Looks/")
    for file in files:
...
assets = [
    "/home/lunar4/jnskkmhr/omn_asset/rock_model/apollo-lunar-sample-1001715_USD/10017-15_SFM_Web-Resolution-Model_Coordinate-Registered.usd",
...
base_height = (
    np.min(data) + 1.0
)  # 0.5 is offset since data - base_height does not give you exact relative position of rock against terrain. (rock is still little bit high)
...
req_ori1 = UserRequest_T(
    p_type=Orientation_T(), sampler=normalmap_clipper1, layer=normalmap_layer1, axes=["x", "y", "z", "w"]
)
...
req_ori2 = UserRequest_T(
    p_type=Orientation_T(), sampler=normalmap_clipper2, layer=normalmap_layer2, axes=["x", "y", "z", "w"]
)
